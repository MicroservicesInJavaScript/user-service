...
const collection = mongoDB.collection(config.mongoCollection);
// Insert some documents
collection.insert(userList, function(err, result) {
    setTimeout(mongoDB.close, 1000);
});

users.list = (url) => new Promise((resolve, reject) => {
    resolve({ entity: JSON.parse(collection) });
})
